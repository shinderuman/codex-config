...
- [x] Codex desktopのbackground `functions.exec`完了outputと後続`functions.wait` result cardで同一terminal payloadが二面表示される親orchestration失敗を解消。長時間cell内ではnested tool outputを変数へ蓄積してrawを描画せずFunctions storeへtask固有keyで保存し、cellはcaptured markerだけを返し、終端後の短い同期loadで全文を1回だけ渡すcontractへ固定した。caller側echo除外をEVALから撤回し、delayed marker・fake providerによる実worker resultのfixed Eval・wiring test・2026-08-21の実structured terminal result単一表示を完了証拠として採用。repo側blind dedupeとworker/reviewer checklistは追加せず、独立review・Sol指摘後再review・全test/race/vet/build/gofmt/install smoke完了
